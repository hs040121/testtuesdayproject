...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Vision%20Transformer-111827?style=flat-square&logoColor=white"/>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white"/>
  <img src="https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white"/>
  <img src="https://img.shields.io/badge/scikit--image-0F172A?style=flat-square&logoColor=white"/>
  <img src="https://img.shields.io/badge/RandomForest-228B22?style=flat-square&logoColor=white"/>
</p>
...
본 프로젝트는 사용자가 업로드한 이미지가 **실제 카메라로 촬영된 사진(Real)** 인지, 혹은 **생성형 AI에 의해 합성된 이미지(Fake)** 인지를 판별하는 웹 기반 AI 시스템을 개발하는 것을 목표로 한다. 본 시스템은 *Fake Image detector*라는 이름으로 구현되었으며, 단일 딥러닝 분류기에 의존하는 기존 방식과 달리 **특징 공학 기반 이미지 포렌식 모델**과 **Vision Transformer(ViT) 기반 딥러닝 모델**을 결합한 **이중 판별 구조(Dual Analysis Engine)** 를 핵심 설계 개념으로 채택하였다.

본 프로젝트의 핵심적인 특징은 단순히 판별 결과만을 제공하는 데 그치지 않고, “왜 해당 이미지가 AI 생성 이미지로 판단되었는가?”에 대한 **분석 근거를 사용자에게 직관적으로 제시**하는 데 있다. 이를 위해 이미지 압축 과정에서 발생하는 오류 분포와 텍스처 패턴의 불일치를 분석하는 **설명 가능한 (AI)** 관점의 접근을 중점적으로 적용하였다.
...
본 장에서는 제안하는 AI 이미지 진위 판별 시스템의 전체 구조와 설계 의도를 설명한다. Figure 2은 본 프로젝트에서 사용한 대규모 학습 데이터와 모델 학습 결과를, Figure 1는 실제 서비스 환경에서 구현된 웹 기반 사용자 인터페이스를 각각 보여준다.
...
본 프로젝트의 실효성을 보다 명확히 검증하기 위해, 동일한 AI 생성 이미지를 대상으로 **본 시스템(Fake Image detector)**과 **상용 AI 이미지 탐지 서비스(Illuminarty)**의 판별 결과를 비교하는 실험을 수행하였다. 비교 대상 이미지는 최신 생성형 AI 모델로 생성된 고품질 이미지로, 육안으로는 실제 사진과 구분이 매우 어려운 사례를 선정하였다.
...
즉, 동일한 AI 생성 이미지에 대해 상용 AI 이미지 탐지 서비스는 AI 생성 가능성을 낮게 평가하였으며, Vision Transformer(ViT) 기반 딥러닝 모델 또한 명확한 판별 근거를 제시하는 데 한계를 보였다.
반면, 본 프로젝트에서 제안한 ELA 및 LBP 기반 포렌식 특징 분석 모델은 이미지의 저수준 통계적 불일치를 효과적으로 포착함으로써, 딥러닝 모델 단독 접근 방식이나 상용 탐지 서비스보다 보다 안정적으로 AI 생성 가능성을 탐지할 수 있었다.
...
또한 본 프로젝트는 단순 정확도 지표를 넘어, **판별 근거를 시각적으로 제시하는 설명 가능한 구조(AI)**를 도입함으로써, 실제 서비스 환경에서의 신뢰성과 활용 가능성을 높였다.
